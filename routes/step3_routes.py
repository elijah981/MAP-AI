...
from database.step3_db import Step3DB
from database.steps_summary_db import StepsSummary
from models.step3_analysis import Step3Analysis

step3_blueprint = Blueprint("step3", __name__)


@step3_blueprint.route("/step3", methods=["GET", "POST"])
def step3():
    ...
    analysis = Step3Analysis()

    if request.method == "POST":
        if "ai_assist" in request.form:
            # Call the step3Analysis function here
            # Code for step3Analysis goes here
            pass
        elif "subject_matter" in request.form:
            # Store the results of the checkboxes in a step3 database
...
@step3_blueprint.route("/step3/analyze_selected_achievement", methods=["POST"])
def analyze_selected_achievement():
    ...
    entry = chatgpt_db.read_entry(summary_id, 3)
    chatgpt_db.close()
    ...
    if entry and "reanalyze" not in reanalyze:
        ...
        if entry:
            chatgpt_db = ChatGPTDB("step_data.db")
            chatgpt_db.create_table()
            chatgpt_db.delete_entry(summary_id, 3)
            chatgpt_db.close()
        ...
        chatgpt_db = ChatGPTDB("step_data.db")
        chatgpt_db.create_table()
        chatgpt_db.add_entry(summary_id, 3, prompt, json.dumps(response), usage)
        chatgpt_db.close()
...
@step3_blueprint.route("/step3/add", methods=["POST"])
def add_step3_data():
    user_id = session.get("selected_user_id")
    step1_summary_id = request.form.get("selected_achievement")
    db = Step3DB("step_data.db")
    db.create_table()

    def generate_checkbox_name(column):
        checkbox_name = f"category_{column.replace('_', '_subcategory_', 1)}_checkbox"
...
@step3_blueprint.route("/step3/summary/count/<user_id>")
...
@step3_blueprint.route("/step3/summary", methods=["POST", "GET"])
def step3_summary():
    ...
    if request.method == "POST":
        ...
        for cat, sub_cats in sub_categories.items():
            ...
            subject_matter[cat] = selections

        # Add the selected checkboxes to the step3_summary table
        ...
        if steps_summary_db.fetch_data(3, selected_user_id):
            steps_summary_db.update_data(
                3, selected_user_id, json.dumps(subject_matter)
            )
        else:
            steps_summary_db.add_data(3, selected_user_id, json.dumps(subject_matter))
        steps_summary_db.close()

    # Code to process and pass the data to the template goes here
    return render_template(
        "step3_summary.html", data=data, sub_categories=sub_categories, totals=totals
    )
