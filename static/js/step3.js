function updateSummaryLabel() {
  ...
  // Fetch the count of analyzed achievements from the step3 database
  fetch(`/step3/summary/count/${user_id}`)
...
function goToSummary() {
  window.location.href = "/step3/summary";
}
...
function analyzeSelectedAchievement() {
  ...
  fetch(`/step3/analyze_selected_achievement`, {
    method: "POST",
    headers: {
...
function populateTableEntries(data) {
  ...
  for (const category in data) {
    ...
    for (const subcategory in subcategories) {
      const checkboxName = `category_${category.toLowerCase()}_subcategory_${subcategory
        .toLowerCase()
        .replace(/ /g, "")
